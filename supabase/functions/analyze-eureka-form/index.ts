...
serve(async (req) => {
  ...
  try {
    ...
    if (!lockResult) {
      console.log('Could not acquire lock - Eureka submission is already being processed or completed');
...
`;
    ...
    console.log('Prompt being sent to OpenAI:', analysisPrompt.substring(0, 500) + '...');
    ...
    console.log('Raw analysis text received from OpenAI:', analysisText.substring(0, 500) + '...');
    ...
    try {
      ...
      console.error('Cleaned analysis text:', analysisText);
