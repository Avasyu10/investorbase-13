...
serve(async (req) => {
  ...
  try {
    ...
    // Process founder LinkedIn URLs
    const founderLinkedInData = submission.founder_linkedin_urls || [];
    let linkedInDataSection = '';
    
    if (founderLinkedInData.length > 0) {
      linkedInDataSection = `
Founder LinkedIn URLs provided: ${founderLinkedInData.join(', ')}
NOTE: Use these LinkedIn URLs to provide specific insights about each founder's background in the team assessment.`;
    }

    // Build analysis prompt with submission data
    const analysisPrompt = `
    You are an expert startup evaluator. Analyze the following Eureka Sample startup application and provide a comprehensive assessment.
    ...
    console.log('Successfully analyzed Eureka Sample submission', submissionId, 'and', isNewCompany ? 'created' : 'updated', 'company', companyId);
