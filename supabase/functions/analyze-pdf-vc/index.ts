...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (analysis.assessmentPoints && analysis.assessmentPoints.length > 0) {
            console.log("Initiating market research with Perplexity API for VC analysis");
            ...
            if (PERPLEXITY_API_KEY) {
              ...
              const doResearch = async () => {
                try {
                  console.log("Starting Perplexity research in background for VC");
                  ...
                  if (!researchResponse.ok) {
                    ...
                    console.log("Research completed successfully for VC");
                  }
                } catch (researchError) {
                  console.error("Error in background research task for VC:", researchError);
                }
              };
          ...
          console.log("Initiating company details extraction with Gemini API for VC");
          const extractDetails = async () => {
            try {
              console.log("Starting company details extraction in background for VC");
              ...
              if (!detailsResponse.ok) {
                ...
                console.log("Company details extraction completed successfully for VC");
              }
            } catch (detailsError) {
              console.error("Error in company details extraction task for VC:", detailsError);
            }
          };
          ...
          console.log("Background tasks initiated for VC");
        } catch (backgroundTasksError) {
          console.error("Error setting up background tasks for VC:", backgroundTasksError);
        ...
        return new Response(
          JSON.stringify({ 
            success: true, 
            companyId,
            message: "VC Report analyzed successfully" 
          }),
          { 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200 
      ...
      console.error("VC Operation error:", error);
    ...
    console.error("Error in analyze-pdf-vc function:", error);
    return new Response(
      JSON.stringify({ 
