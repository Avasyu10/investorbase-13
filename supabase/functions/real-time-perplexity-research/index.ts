...
serve(async (req)=>{
  ...
  try {
    ...
    const assessmentPoints = Array.isArray(requestData.assessmentPoints) ? requestData.assessmentPoints : [
    ...
    const SUPABASE_ANON_KEY = Deno.env.get('SUPABASE_ANON_KEY') || ''; // Although not used here, good to keep for client-side
    ...
    const userPrompt = `You are a top-tier venture capital (VC) analyst providing comprehensive research about ${companyName}. Based on the following assessment of the company, I need you to conduct deep market research and provide detailed analysis focusing on the LATEST and MOST RELEVANT market information.
...
- NEVER repeat the same information across different sections`;
    ...
    console.log("[DEBUG] Prompt:", fullPrompt.substring(0, 200) + "...");
    ...
    // Keeping column names as 'perplexity_' to avoid DB migration, but consider renaming to 'gemini_' or 'llm_'
    const { error: updateCompanyError } = await supabase.from('companies').update({
      ...
      perplexity_prompt: fullPrompt // Use the full prompt sent to Gemini
    }).eq('id', companyId);
    ...
    const { data: researchData, error: insertResearchError } = await supabase.from('market_research').insert({
      ...
      prompt: fullPrompt // Use the full prompt sent to Gemini
    }).select().single();
...
function extractResearchSummary(text) {
  try {
...
function extractNewsHighlights(text) {
  ...
  const newsMatches = [
    ...newsContent.matchAll(newsItemRegex)
  ];
...
function extractMarketInsights(text) {
  ...
  let numberedMatches = [
    ...insightsContent.matchAll(numberedInsightRegex)
  ];
  if (numberedMatches.length > 0) {
    for (const match of numberedMatches){
      ...
      if (!url) {
        ...
        url = parenthesesUrlUrlMatch ? parenthesesUrlMatch[1].trim() : "";
      }
  ...
  if (marketInsights.length === 0) {
    ...
    let regularMatches = [
      ...insightsContent.matchAll(regularInsightRegex)
    ];
    for (const match of regularMatches){
      ...
      if (contentMatch && contentMatch[1]) {
        ...
        content = lines.filter((line)=>!line.includes('**Source:**') && !line.includes('**URL:**')).join(' ').trim();
      }
      // Extract URL
      const urlMatch = insightText.match(/\*\*URL:\*\*\s*(https?:\/\/[^\s\n\]]+)/i);
      let url = urlMatch ? urlMatch[1].trim() : "";
  ...
  if (marketInsights.length === 0) {
    const listItemRegex = /(?:^|\n)(\d+\.\s+\*\*[^*]+\*\*)[\s\S]*?(?=\n\d+\.\s+\*\*|\n## |$)/g;
    let listMatches = [
      ...insightsContent.matchAll(listItemRegex)
    ];
  ...
  if (marketInsights.length === 0) {
    ...
    for(let i = startIndex; i < paragraphs.length; i++){
      ...
      const headline = firstSentence.length > 60 ? firstSentence.substring(0, 57) + '...' : firstSentence;
