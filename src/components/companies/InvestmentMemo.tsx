...
export const InvestmentMemo: React.FC<InvestmentMemoProps> = ({
  ...
  // Also removes markdown bolding `**` for PDF rendering
  const formatAsBulletPoints = (text: string | string[]): string[] => {
    let cleanedText: string | string[];
    if (Array.isArray(text)) {
      cleanedText = text.map(item => item.replace(/\*\*(.*?)\*\*/g, '$1').trim()); // Remove ** **
    } else {
      cleanedText = text.replace(/\*\*(.*?)\*\*/g, '$1').trim(); // Remove ** **
    }
    if (Array.isArray(cleanedText)) {
      return cleanedText.map(item => `• ${item}`).filter(item => item.length > 2);
    }
  ...
  const getRisksContent = (): string[] => {
    ...
    // Markdown bolding is handled by formatAsBulletPoints
  ...
  const downloadAsPDF = () => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(`INVESTMENT`, centerX, yPosition + 50, {
      align: 'center'
    });
    doc.text(`MEMORANDUM`, centerX, yPosition + 65, {
      align: 'center'
    }); // Split for better visual
    yPosition += 90;
    doc.setFontSize(30); // Larger company name
    doc.text(company.name.toUpperCase(), centerX, yPosition + 30, {
      align: 'center'
    });
    yPosition += 40;
    doc.setFontSize(16);
    doc.setFont('helvetica', 'normal');
    doc.text(`A comprehensive analysis for potential investors`, centerX, yPosition + 20, {
      align: 'center'
    });
    yPosition += 80;
  ...
  return <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        <Button variant="outline" className="flex items-center gap-2 border-gray-300 text-gray-800 bg-yellow-500 hover:bg-yellow-400">
