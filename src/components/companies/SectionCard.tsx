...
export const SectionCard = ({ section, onClick }: SectionCardProps) => {
  ...
  return (
    <Card 
      className="cursor-pointer hover:shadow-lg transition-all duration-200 border-0 shadow-subtle hover:scale-105"
      onClick={onClick}
    >
              ...
              <div className="text-xs">
                ...
                <div className="text-green-600 mt-1">
                  {section.strengths.slice(0, 2).map((strength, idx) => (
                    <div key={idx} className="truncate">• {strength}</div>
                  ))}
                  {section.strengths.length > 2 && (
                    <div className="text-green-500">+{section.strengths.length - 2} more</div>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="text-xs">
                ...
                <div className="text-amber-600 mt-1">
                  {section.weaknesses.slice(0, 2).map((weakness, idx) => (
                    <div key={idx} className="truncate">• {weakness}</div>
                  ))}
                  {section.weaknesses.length > 2 && (
                    <div className="text-amber-500">+{section.weaknesses.length - 2} more</div>
                  )}
                </div>
              </div>
            </div>
