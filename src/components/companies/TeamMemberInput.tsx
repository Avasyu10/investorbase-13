...
export function TeamMemberInput({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const {
        ...
        error: fetchError
      } = await supabase.from('company_details').select('id').eq('company_id', companyId).maybeSingle();
      if (fetchError) {
        throw fetchError;
      }
      if (existingDetails) {
        ...
        const {
          error: updateError
        } = await supabase.from('company_details').update({
  ...
  if (isEditing) {
    return <div className="flex items-center gap-1 w-full">
        <Input value={teamMemberName} onChange={e => setTeamMemberName(e.target.value)} placeholder="Enter team member name" className="h-8 text-xs" disabled={isUpdating} />
        <Button variant="ghost" size="sm" onClick={handleSave} disabled={isUpdating} className="h-8 w-8 p-0 text-green-600 hover:text-green-700 hover:bg-green-50">
          {isUpdating ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3 w-3" />}
        </Button>
        <Button variant="ghost" size="sm" onClick={handleCancel} disabled={isUpdating} className="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-red-50">
          <X className="h-3 w-3" />
        </Button>
      </div>;
  }
  return <div className="flex items-center gap-1 group">
