...
// amber-500
'#8b5cf6',
// violet-500 (kept for other potential uses, but not directly used for the violet replacement)
'#06b6d4',
// cyan-500
'#84cc16',
// lime-500
...
export function VCDashboard() {
  ...
  const industryData = Object.entries(industryDistribution).map(([industry, count], index) => ({
    industry,
    count,
    fill: CHART_COLORS[index % CHART_COLORS.length]
  })).sort((a, b) => b.count - a.count).slice(0, 8);
  const topCompanies = [...companies].sort((a, b) => b.overall_score - a.overall_score).slice(0, 10);
  const trendData = [{
    month: 'Jan',
    avgScore: 68,
    companies: 12
  ...
  const averageScore = companies.length > 0 ? (companies.reduce((sum, c) => sum + c.overall_score, 0) / companies.length).toFixed(1) : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <div className="mt-4">
              ...
              <select id="person-select" value={selectedPerson} onChange={e => setSelectedPerson(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md dark:border-gray-700 dark:text-white bg-slate-950">
                {Object.keys(channelDataByPerson).map(personName => <option key={personName} value={personName}>
                    {personName}
                  </option>)}
              </select>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={meetingCategoriesData} cx="50%" cy="50%" innerRadius={60} outerRadius={120} paddingAngle={5} dataKey="value">
                  {meetingCategoriesData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
                </Pie>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <RadialBarChart cx="50%" cy="50%" innerRadius="20%" outerRadius="80%" data={industryData}>
                ...
                <Legend iconSize={10} layout="vertical" verticalAlign="middle" align="right" formatter={(value, entry) => {
                const item = entry.payload;
                return item ? `${item.industry}: ${item.count}` : `${value}`;
              }} />
                <ChartTooltip formatter={(value, name, props) => [`${value} companies`, props.payload.industry]} />
              </RadialBarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={trendData}>
                ...
                <YAxis domain={[60, 80]} />
                <ChartTooltip />
                <Area type="monotone" dataKey="avgScore" stroke={CHART_COLORS[2]} fill={CHART_COLORS[2]} fillOpacity={0.3} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {topCompanies.map((company, index) => <div key={company.id} className="flex items-center justify-between p-4 border rounded-lg">
