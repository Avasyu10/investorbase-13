...
export default function BarcApplications() {
  ...
  const fetchSubmissions = async (page: number = currentPage, filter: string = statusFilter) => {
    ...
    try {
      console.log('Fetching BARC submissions for user:', user.id, 'page:', page, 'filter:', filter);
      ...
      const filteredQuery = addStatusFilter(barcBaseQuery);
      const offset = (page - 1) * pageSize;
      const { data: barcSubmissions, error, count } = await filteredQuery
      ...
      setSubmissions(mappedBarcSubmissions);
      ...
      toast({
        title: "Error",
        description: "Failed to fetch BARC applications. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const barcChannel = supabase
      .channel('barc-submissions-page-realtime')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">BARC Applications ({totalApplications || 0})</h2>
          <p className="text-muted-foreground">
            BARC form submissions across all your forms
          </p>
        </div>
      ...
      {submissions.length === 0 ? (
        <div className="text-center py-12 border rounded-lg bg-card/50">
          ...
          <h3 className="mt-4 text-lg font-medium">No BARC applications found</h3>
