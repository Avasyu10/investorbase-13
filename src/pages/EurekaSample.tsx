...
import { Loader2, Building, Plus, X, ExternalLink } from "lucide-react";
...
// Iframe component that reuses the same submission logic
const EurekaIframe = ({ slug, onSubmit }: { slug: string, onSubmit: (data: EurekaFormData, founderLinkedIns: string[]) => Promise<void> }) => {
  ...
  const handleIframeSubmit = async (data: EurekaFormData) => {
    console.log('📝 Iframe Eureka form submit triggered:', data);
    ...
    try {
      await onSubmit(data, founderLinkedIns);
      form.reset();
      setFounderLinkedIns([""]);
    } catch (error: any) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border min-h-screen">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleIframeSubmit)} className="space-y-4">
...
const EurekaSample = () => {
  ...
  // Shared submission logic that both regular form and iframe can use
  const handleSharedSubmit = async (data: EurekaFormData, linkedInProfiles: string[]) => {
    console.log('📋 Shared submission logic triggered:', data);
    ...
    const submissionData: EurekaSubmissionData = {
      ...
      founder_linkedin_urls: linkedInProfiles.filter(url => url.trim()),
    ...
    console.log('📋 Final shared submission data:', submissionData);
    ...
    console.log('📋 Shared form submitted successfully:', submission);
  ...
  const onSubmit = async (data: EurekaFormData) => {
    console.log('📝 Regular form submit triggered:', data);
    ...
    console.log('✅ Regular form validation passed, submitting...');
    setIsSubmitting(true);

    try {
      await handleSharedSubmit(data, founderLinkedIns);
      form.reset();
      setFounderLinkedIns([""]);
    } catch (error: any) {
      console.error('❌ Error submitting regular form:', error);
      toast({
        title: "Submission Error",
        description: `There was an error submitting your application: ${error.message || 'Please try again.'}`,
        variant: "destructive",
      ...
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            <EurekaIframe 
              slug={slug || 'eureka-sample'} 
              onSubmit={async (data, linkedInProfiles) => {
                setIsSubmitting(true);
                try {
                  await handleSharedSubmit(data, linkedInProfiles);
                } finally {
                  setIsSubmitting(false);
                }
              }}
